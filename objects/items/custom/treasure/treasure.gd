...
func collect() -> void:
	if is_instance_valid(Util.get_player()):
		Util.get_player().quick_heal(maxi(roundi(float(get_heal_value() / 3)
		Globals.healed_from_treasure += floor(get_heal_value()) * 0.333), 1) * Util.get_player().stats.get_stat("healing_effectiveness")) 
