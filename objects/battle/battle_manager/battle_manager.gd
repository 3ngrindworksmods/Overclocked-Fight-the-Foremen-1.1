...
func get_damage(damage: float, action: BattleAction, target: Node3D) -> int:
	...
	if user is Player:
		...
		if action is GagLure:
			...
			target.last_damage_source = user_stats.character.gag_loadout.get_action_track(action).track_name
			gag_type = user_stats.character.gag_loadout.get_action_track(action).track_name
